pipeline {
    ...
    stages {
        ...
        stage ('Deploy Backend') {
            steps {
                deploy adapters: [tomcat9( url: 'http://localhost:8001/')], contextPath: 'tasks-backend', war: 'target/tasks-backend.war'
            }
        }
        stage ('API Test') {
            steps {
        ...
        stage ('Deploy Frontend') {
            steps {
                dir('frontend') {
                    ...
                    deploy adapters: [tomcat9( url: 'http://localhost:8001/')], contextPath: 'tasks', war: 'target/tasks.war'
                }
            }
        }
